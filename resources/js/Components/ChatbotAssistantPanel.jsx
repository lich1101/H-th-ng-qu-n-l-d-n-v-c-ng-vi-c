...
export default function ChatbotAssistantPanel({
    ...
    return (
        <div className={embedded ? '' : 'lg:col-span-2'}>
            <div className={`border border-slate-200/80 bg-white/90 shadow-[0_24px_60px_-32px_rgba(15,23,42,0.45)] ${embedded ? 'rounded-[20px] bg-gradient-to-b from-white to-slate-50/35 p-2' : 'rounded-[24px] p-3'}`}>
                <div className={`grid ${embedded ? 'grid-cols-1 gap-3.5' : 'gap-5 xl:grid-cols-[minmax(0,1.85fr)_minmax(320px,1fr)]'}`}>
                        <div className={`border border-slate-200/80 bg-white ${embedded ? 'rounded-[18px]' : 'rounded-[20px] shadow-card'}`}>
                            <div className={`border-b border-slate-200/80 ${embedded ? 'px-3.5 py-3' : 'px-5 py-4'}`}>
                            <div className="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                    <h3 className="text-base font-semibold text-slate-900">{embedded ? 'Hội thoại nhanh' : 'Hội thoại của bạn'}</h3>
                                    <p className={`mt-1 ${embedded ? 'text-[11px]' : 'text-sm'} text-text-muted`}>{headerDescription}</p>
                                </div>
                            ...
                            <div className={`mt-2.5 flex flex-col gap-2.5 ${embedded ? 'sm:flex-row sm:items-center sm:justify-between' : 'lg:flex-row lg:items-center lg:justify-between'}`}>
                                ...
                                <div className={`w-full ${embedded ? 'max-w-full sm:max-w-[280px]' : 'max-w-[340px]'}`}>
                                    <select
                            ...
                            <div
                                ref={scrollRef}
                                className={`space-y-4 overflow-y-auto bg-gradient-to-b from-slate-50 via-white to-slate-100/70 ${embedded ? 'h-[36vh] min-h-[230px] max-h-[360px] px-3.5 py-3.5' : 'h-[56vh] min-h-[360px] max-h-[620px] px-5 py-5'}`}
                            >
                            {loading ? (
                            ...
                            <div className={`border-t border-slate-200/80 bg-gradient-to-b from-slate-50/70 to-white ${embedded ? 'px-3.5 py-3' : 'px-5 py-4'}`}>
                        ...
                        <div className={`space-y-4 ${embedded ? '' : 'xl:sticky xl:top-4 xl:self-start'}`}>
                            <div className={`border border-slate-200/80 bg-white ${embedded ? 'rounded-[18px] p-3.5' : 'rounded-[20px] p-4 shadow-card'}`}>
